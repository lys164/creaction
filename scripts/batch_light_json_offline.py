# -*- coding: utf-8 -*-
"""批量用「轻剧情(light track)」链路跑角色 —— 本地(离线)生成，数据双写线上。

为什么离线：线上服务器磁盘 100% 满，上传大图触发 nginx 500 落盘失败。本地直接
调 pipeline 出人设，人设 JSON 经 storage.save_json 写本地 + arca 存储中台（线上），
源图经 storage.save_file 写本地 + OSS（火山 TOS，独立于服务器磁盘）——全程不碰
服务器磁盘，线上服务器随即可查到（list_json 会合并 arca 远端记录）。

需求（同 online 版）：
- 输入若干 JSON（小红书导出），一个 item = 一个角色。
- 每个角色只拼入该 item 的 desc（创作补充要求）+ images 的【所有】图片（视觉输入）。
- track=light（轻剧情），中日韩英四语言各生成一个本土化角色，source="mengnv"。
- 只生成角色（人设），不出封面、不出帖子。

断点续跑：进度写 data/batch_light_json_offline_state.json；首次会并入 online 版
已完成的 item（避免重复生成第一个）。
...
def load_state() -> dict:
    try:
        ...
        pass
    # 首次：并入 online 版已完成，避免重复生成第一个 item
    seed_done: list[str] = []
    try:
...
def _download_all(urls: list[str], key: str) -> list[str]:
    """下载 item 的所有图片，storage.save_file 落本地 uploads + OSS。返回本地路径。"""
    paths: list[str] = []
    ...
    for i, u in enumerate(urls):
        ...
        try:
            ...
            if not r.ok or not r.content:
                print(f"      ⚠ 图片下载失败 {r.status_code}: {u[:70]}", flush=True)
                continue
...
def main() -> int:
    ...
    if not storage.arca_storage.enabled():
        print("✗ arca 存储未启用，本地数据不会同步到线上（需 ARCA_STORAGE_KEY）")
        return 2

    print(f"本地生成 → 双写线上 arca 中台 ({config.ARCA_BASE_URL})")
    ...
    if args.dry_run:
        for key, desc, imgs in todo[:5]:
            print(f"  样例: imgs={len(imgs)} desc={desc[:36]!r} key={key[:56]}")
    ...
    def _run(job):
        ...
        try:
            ...
            with _STATE_LOCK:
                counters["err"] += 1
            print(f"      ✗ 失败[{idx}]: {type(e).__name__}: {e}", flush=True)
