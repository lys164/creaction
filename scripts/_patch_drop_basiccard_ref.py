# -*- coding: utf-8 -*-
"""刪除封面規範裡對「基礎卡」的外部指代（模型無從得知基礎卡是什麼），
只保留自包含的硬數值規格。逐語言精確替換，冪等。"""
import json
...
REPL = {
    "zh-CN": ("豎構圖人物封面，嚴格對齊基礎卡規範——封面圖容器固定 aspect-ratio:4/4.6",
              "豎構圖人物封面。封面圖容器固定 aspect-ratio:4/4.6"),
    ...
    "ja": ("縦構図の人物カバー。ベーシックカード規範に厳密に合わせる——カバー畫像コンテナは aspect-ratio:4/4.6",
           "縦構図の人物カバー。カバー畫像コンテナは aspect-ratio:4/4.6"),
}
...
def main() -> int:
    ...
    for lang, (old, new) in REPL.items():
        ...
        if old in sp:
            ...
            print(f"[skip] {lang} 已處理")
        else:
            print(f"[WARN] {lang} 未命中舊句")
    # 頂層(與 zh-CN 同源)
    old, new = REPL["zh-CN"]
