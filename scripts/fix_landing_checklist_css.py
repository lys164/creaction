# -*- coding: utf-8 -*-
"""给已生成的「互动卡片版」落地页补上缺失的 `.check` 勾选清单 CSS（确定性修复）。

背景：互动版落地页是自包含整份 HTML，CSS 在生成时从母版复刻并烘焙进页面。早期母版
里 A 交互「完成揭示」用了 `<ul class="check"><li data-tick><i></i><span>` 结构，但没有
配套 `.check` CSS —— 于是浏览器按默认 <ul> 圆点渲染、方框记号不可见、行也不跟
textlist/info 行系统对齐（就是「样式不符合规范」）。母版已补上 `.check` 规则，本脚本
把同一段 CSS 注入到存量页面里，不走 LLM、不改文案、天然幂等。

与 reskin_landing_interactive.py 的区别：那个是 LLM 重新生成（慢、重掷内容）；本脚本
只做一次字符串注入 + PUT 保存，快且确定。

判定与跳过：
- variant != interactive          → 跳过（默认页无此结构）
- 页面里没有 class="check"        → 跳过（该页用的是 B/C 交互，无清单）
- 已含 `.check li{`               → 跳过（已修复，幂等）
- 找不到注入锚点 .textrow .note{} → 记为 skipped_no_anchor，留待人工核查

用法：
  python3 scripts/fix_landing_checklist_css.py --dry-run
  python3 scripts/fix_landing_checklist_css.py            # 实跑
...
# 注入锚点：母版基础 CSS 里 textlist 索引清单的最后一行，`.check` 紧随其后。
ANCHOR = ('.textrow .note{font-size:var(--fs-note);color:var(--sub);'
          'line-height:1.68;margin:5px 0 0 34px;letter-spacing:-.01em}')

# 要注入的 `.check` 勾选清单样式（与 landing_prompts.json 互动版母版逐字一致）。
CHECK_CSS = (
    "\n"
    "/* 完成揭示 · 勾选清单（对齐 textlist / info 行系统：细线行 + 方框记号） */\n"
...
def _needs_fix(html: str) -> str:
    """返回该页处置：fix / skip_no_check / already / no_anchor。"""
...
def main() -> int:
    ...
    ap.add_argument("--source", action="append", dest="sources",
                    help="要修的 source，可多次传；默认 chouxiang/feiren/heermeng")
    ...
    print(f"扫描 source: {', '.join(sources)}  → 目标：补 .check 勾选清单 CSS")
    ...
    print(f"完整角色 {len(all_ids)}，状态已完成 {len(all_ids) - len(todo_ids)}，"
          f"待分类 {len(todo_ids)}")
    ...
    print(f"\n本轮需要注入 CSS 的页面: {len(to_fix)}")

    # 把非 fix 的确定性归档进 state（幂等，续跑不再重复分类）
    with _STATE_LOCK:
    ...
    if args.dry_run:
        for cid, _ in to_fix[:8]:
            print(f"  [DRY] 将修复 {cid}")
        print(f"[DRY] 计划给 {len(to_fix)} 个页面注入 .check CSS。"
