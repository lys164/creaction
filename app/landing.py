...
def build_user_message(persona: dict, lang: str, has_cover: bool,
    ...
    if post_images:
        n = len(post_images)
        lines = [
            f"\n# TA 的 {n} 张真实生活照片（已作为图片发给你，可见）——【次要补充素材，不是页面主角】：",
            "定位：这页的**主体始终是「人物介绍」**（名称/身份/特质/与观者的关系/钩子按 §二 完整呈现），"
            "封面图与人物信息才是视觉重心。这些生活照只是用来点缀、增加生活实感的【辅料】，"
            "请**克制使用**：只在介绍信息已铺满后，用一个不抢戏的小模块（如一处小相册/几张拼贴小图/"
            "动态流缩略图）收纳即可，**严禁让照片占据画面主要面积、严禁整页都是照片九宫格**。",
            "占比硬约束：所有帖子图加起来占画面面积建议 ≤ 30%，单张不要做成大图主视觉；"
            "可以只选用其中 2-4 张更贴合氛围的，不必全放。它们服务于人物介绍，不能喧宾夺主。",
